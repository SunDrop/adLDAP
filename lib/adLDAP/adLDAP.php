...
class adLDAP
{
    ...
    function __construct(array $options = [])
    {
        ...
        if (count($options) > 0) {
            ...
            if (array_key_exists("follow_referrals", $options)) {
                $this->followReferrals = $options["follow_referrals"];
            }
        ...
        if (!$this->connect()) {
            throw new adLDAPException('Can\'t connect to LDAP server');
        }
    }
    ...
    public function connect()
    {
        ...
        if ($this->adminUsername !== null && $this->adminPassword !== null) {
            $this->ldapBind = @ldap_bind($this->ldapConnection, $this->adminUsername . $this->accountSuffix,
                $this->adminPassword);
            if (!$this->ldapBind) {
     ...
     * @param bool $preventRebind optional
     * @return bool
     */
    ...
    public function search($includeDescription = false, $search = "*", $sorted = true)
    {
        ...
        $fields = ["cn", "description", "displayname", "distinguishedname", "samaccountname"];
        ...
        for ($i = 0; $i < $entries["count"]; $i++) {
            if ($includeDescription && strlen($entries[$i]["description"][0]) > 0) {
                $objectArray[$entries[$i]["samaccountname"][0]] = [
                    $entries[$i]["cn"][0],
