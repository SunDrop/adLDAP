...
class adLDAPFolders
{
    ...
    public function listing($folderName = null, $dnType = adLDAP::ADLDAP_FOLDER, $recursive = null, $type = null)
    {
        ...
        if ($recursive === true) {
            $sr = ldap_search($this->adldap->getLdapConnection(), $searchOu, $filter,
            ...
            $sr = ldap_list($this->adldap->getLdapConnection(), $searchOu, $filter,
                ['objectclass', 'distinguishedname', 'samaccountname']);
            $entries = @ldap_get_entries($this->adldap->getLdapConnection(), $sr);
            if (is_array($entries)) {
                return $entries;
    ...
    public function create($attributes)
    {
        ...
        $add = [ 'objectClass' => 'organizationalUnit', 'OU' => $attributes['ou_name'] ];
        $containers = 'OU=' . implode(',OU=', $attributes['container']);
        $result = ldap_add($this->adldap->getLdapConnection(),
            'OU=' . $add['OU'] . ', ' . $containers . $this->adldap->getBaseDn(), $add);
        if ($result != true) {
            return false;
        }
        return true;
