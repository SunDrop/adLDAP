...
class adLDAPGroups
{
    ...
    public function addGroup($parent, $child)
    {
        ...
        $parentGroup = $this->ginfo($parent, ['cn']);
    ...
    public function create($attributes)
    {
        ...
        $result = ldap_add($this->adldap->getLdapConnection(),
            'CN=' . $add['cn'] . ', ' . $container . ',' . $this->adldap->getBaseDn(), $add);
        if ($result != true) {
            return false;
        }
        return true;
    ...
    public function rename($group, $newName, $container)
    {
        ...
        // Do the update
        $result = @ldap_rename($this->adldap->getLdapConnection(), $groupDN, $newRDN,
            $container . ', ' . $this->adldap->getBaseDn(), true);
        if ($result == false) {
            return false;
        }
        return true;
    ...
    {
        // Find the parent dn
        $parentGroup = $this->info($parent, ['cn']);
        if ($parentGroup[0]['dn'] === null) {
            return false;
        }
        $parentDn = $parentGroup[0]['dn'];
    ...
    public function inGroup($group, $recursive = null)
    {
        ...
        $info = $this->info($group, ['member', 'cn']);
        ...
        if (isset($entries[0]['member;range=0-1499']) && $entries[0]['member;range=0-1499']['count'] == 1500) {
            ...
            do {
                $sr = ldap_search($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), $filter,
                    ['member;range=' . $rangelow . '-' . $rangehigh]);
     ...
     * @param string $groupName The group name to retrieve info about
     * @param array $fields Fields to retrieve
     * @return bool|\adLDAP\collections\adLDAPGroupCollection
     */
    ...
    public function recursiveGroups($group)
    {
        ...
        while (count($stack) > 0) {
            ...
            $info = $this->info($parent, ['memberof']);
     ...
     *
     * @param int|string $sAMAaccountType The account type to return
    ...
    public function search(
        $sAMAaccountType = adLDAP::ADLDAP_SECURITY_GLOBAL_GROUP,
