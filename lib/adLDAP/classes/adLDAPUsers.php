...
class adLDAPUsers
{
     ...
     * @internal param bool $prevent_rebind optional
     */
    ...
    public function create($attributes)
    {
        ...
        // Add the entry
        $result = @ldap_add($this->adldap->getLdapConnection(),
            'CN=' . $add['cn'][0] . ', ' . $container . ',' . $this->adldap->getBaseDn(), $add);
        if ($result != true) {
            return false;
        }
        return true;
    ...
    {
        $userinfo = $this->info($username, ['*'], $isGUID);
    ...
    public function groups($username, $recursive = null, $isGUID = false)
    {
        ...
        $info = @$this->info($username, ['memberof', 'primarygroupid'], $isGUID);
            ...
            if ($entries[0]['count'] >= 1) {
                if (in_array('memberof', $fields)) {
                    ...
                    if ($this->adldap->getRealPrimaryGroup() && isset($entries[0]['primarygroupid'][0]) && isset($entries[0]['objectsid'][0])) {
                        ...
                        $entries[0]['memberof'][] = $this->adldap->group()->getPrimaryGroup($entries[0]['primarygroupid'][0],
                            $entries[0]['objectsid'][0]);
                    } else {
    ...
    public function passwordExpiry($username, $isGUID = false)
    {
        ...
        $userInfo = $this->info($username, ['pwdlastset', 'useraccountcontrol'], $isGUID);
        ...
        $sr = ldap_read($this->adldap->getLdapConnection(), $this->adldap->getBaseDn(), 'objectclass=*', ['maxPwdAge']);
        if (!$sr) {
            return false;
        }
    ...
    {
        $user = $this->info($username, ['cn'], $isGUID);
        if ($user[0]['dn'] === null) {
            return false;
        }
        $userDn = $user[0]['dn'];
     ...
     *
     * @param bool $includeDescription Return a description of the user
     * @param bool|string $searchField Field to search search for
     * @param bool|string $searchFilter Value to search for in the specified field
     * @param bool $sorted Sort the user accounts
     * @return array
     */
