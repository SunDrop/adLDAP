...
class adLDAPExchange
{
     ...
     * @internal param bool $mdbUseDefaults Indicates whether the store should use the default quota, rather than the per-mailbox quota.
     */
    ...
    public function deleteAddress($username, $emailAddress, $isGUID = false)
    {
        ...
        if (is_array($user[0]["proxyaddresses"])) {
            $mod = [];
            for ($i = 0; $i < $user[0]['proxyaddresses']['count']; $i++) {
                if (strpos($user[0]['proxyaddresses'][$i],
                        'SMTP:') !== false && $user[0]['proxyaddresses'][$i] == 'SMTP:' . $emailAddress
                ) {
                    $mod['proxyAddresses'][0] = 'SMTP:' . $emailAddress;
                } elseif (strpos($user[0]['proxyaddresses'][$i],
                        'smtp:') !== false && $user[0]['proxyaddresses'][$i] == 'smtp:' . $emailAddress
                ) {
                    $mod['proxyAddresses'][0] = 'smtp:' . $emailAddress;
                }
    ...
    public function primaryAddress($username, $emailAddress, $isGUID = false)
    {
        ...
        if (is_array($user["proxyaddresses"])) {
            $modAddresses = [];
            for ($i = 0; $i < $user['proxyaddresses']['count']; $i++) {
                if (strpos($user['proxyaddresses'][$i], 'SMTP:') !== false) {
                    $user['proxyaddresses'][$i] = str_replace('SMTP:', 'smtp:', $user['proxyaddresses'][$i]);
                }
                if ($user['proxyaddresses'][$i] == 'smtp:' . $emailAddress) {
                    $user['proxyaddresses'][$i] = str_replace('smtp:', 'SMTP:', $user['proxyaddresses'][$i]);
                }
                if ($user['proxyaddresses'][$i] != '') {
                    $modAddresses['proxyAddresses'][$i] = $user['proxyaddresses'][$i];
                }
            }
     ...
     * @return array
     */
    public function servers($attributes = ['cn', 'distinguishedname', 'serialnumber'])
    {
        ...
        $sr = @ldap_search($this->adldap->getLdapConnection(),
            $configurationNamingContext[0]['configurationnamingcontext'][0], '(&(objectCategory=msExchExchangeServer))',
            $attributes);
